...
public class UIController : MonoBehaviour
{
    ...
    [SerializeField] private CanvasGroup shootModeCanvasGroup;  // Thêm CanvasGroup để kiểm soát alpha
    ...
    private IEnumerator FadeOutLabel(float duration)
    {
        ...
        {
            shootModeCanvasGroup.alpha = 1 - (elapsedTime / duration); // Giảm alpha từ 1 đến 0
            elapsedTime += Time.deltaTime;
            yield return null;
        }
        shootModeCanvasGroup.alpha = 0f; // Đảm bảo alpha cuối cùng là 0
        shootModeLabel.gameObject.SetActive(false); // Ẩn label khi fade out hoàn tất
    }
}
